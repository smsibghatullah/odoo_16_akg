...
import { click as clickWait, contains } from "@web/../tests/utils";
...
QUnit.module('documents_kanban_tests.js', {
    ...
    QUnit.test(
        "navigating to another document in preview should deselect the current one upon closing the preview",
        async function (assert) {
            ...
            await click(target, '.o_search_panel_category_value[title="All"] header');
            await click(find(target, '.o_kanban_record', 'shut'));
            ...
            await click(
                target,
                '.o_AttachmentViewer_buttonNavigation[title="Next (Right-Arrow)"]'
            );
            const record = find(target, '.o_kanban_record', 'nir');
            assert.hasClass(record, "o_record_selected");
            await click(target, '.o_AttachmentViewer_headerItemButtonClose');
            assert.doesNotHaveClass(record, "o_record_selected");
        }
    );
    ...
    QUnit.test("SearchPanel: preview is closed automatically when changing workspace/tag/facet", async function (assert) {
        assert.expect(27); // 10 clicks with parent (x 2) + 7 contains (x 1)
        pyEnv["documents.document"].create({
            folder_id: pyEnv["documents.folder"].create({ name: "WorkspaceYoutube", has_write_access: true }),
        ...
        const workspaceYoutube = [".o_search_panel_label",
            { parent: [".o_search_panel_category_value", { text: "WorkspaceYoutube" }] }];
        const workspace2 = [".o_search_panel_label",
            { parent: [".o_search_panel_category_value", { text: "Workspace2" }] }];
        const facetStatus = ["input.form-check-input",
            { parent: [".o_search_panel_group_header", { text: "●Status" }] }];
        const tagDraft = ["input.form-check-input",
            { parent: [".o_search_panel_filter_value", { text: "Draft" }] }];
        const newYoutubeVideoDocumentPreview = [".oe_kanban_previewer",
            { parent: [".o_kanban_record", { text: "newYoutubeVideo" }] }];
        const openPreview = async () => {
            await clickWait(...workspaceYoutube);
            await clickWait(...newYoutubeVideoDocumentPreview);
            await contains(".o_AttachmentViewer");
        };

        // Changing workspace must close the preview
        await openPreview();
        await clickWait(...workspace2);
        await contains(".o_AttachmentViewer", { count: 0 });
        // Selecting a facet must close the preview
        await openPreview();
        await clickWait(...facetStatus);
        await contains(".o_AttachmentViewer", { count: 0 });
        await contains(".o_kanban_record", { count: 0, text: "newYoutubeVideo" });
        await clickWait(...facetStatus); // Unselect the facet to make the document visible again for the next test
        // Selecting a tag must close the preview
        await openPreview();
        await clickWait(...tagDraft);
        await contains(".o_AttachmentViewer", { count: 0 });
    });
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await createDocumentsView({
                ...
                mockRPC: function (route, args) {
                    if (args.method === "write" && args.model === "documents.document") {
                        throw new Error('It should not be possible to drop a record into the same/current workspace');
                    }
                },
            });

            const yopRecord = find(target, ".o_kanban_record", "yop");
            const yopRecordSelector = yopRecord.querySelector(".o_record_selector");
            // selects the record
            await click(yopRecordSelector);
            await nextTick();
