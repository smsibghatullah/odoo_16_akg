...
QUnit.module('ViewEditorManager', {
    ...
    QUnit.test("notebook with empty page and fields inside the element", async (assert) => {
        ...
        assert.strictEqual(target.querySelector(".o_form_sheet .o_notebook_headers li:nth-child(2)").dataset.studioXpath, "/form[1]/sheet[1]/notebook[1]/page[2]");
    });
    ...
    QUnit.test("cannot add a related properties field", async (assert) => {
        serverData.models.coucou.fields.m2o = { type: "many2one", relation: "product", string: "m2o to product"};
        ...
        await testUtils.dom.dragAndDrop(
            $(target.querySelector(".o_web_studio_sidebar_content .o_web_studio_field_related")),
            $(target.querySelector(".o_web_studio_form_view_editor .o_web_studio_hook"))
        );
        assert.containsOnce(target, ".modal .o_web_studio_field_dialog_form")

        document.body.querySelector(".modal .o_field_selector").dispatchEvent(new Event("focus", {bubbles: true}));
        await nextTick();
        await testUtils.dom.click(document.body.querySelector(".o_field_selector_item[data-name='m2o']"));
        await nextTick();
        assert.deepEqual(
            [...document.body.querySelectorAll(".o_field_selector_page .o_field_selector_item")].map(el => el.textContent.trim()),
    ...
    QUnit.test('Restrict drag and drop of notebook and group in a inner group', async function (assert) {
        ...
        await testUtils.dom.dragAndDrop(vem.el.querySelector('.o_web_studio_field_type_container .o_web_studio_field_tabs'), vem.el.querySelector('.o_inner_group .o_web_studio_hook'));
        assert.strictEqual(editViewCount, 0,
            "the notebook cannot be dropped inside a group");
        await testUtils.dom.dragAndDrop(vem.el.querySelector('.o_web_studio_field_type_container .o_web_studio_field_columns'), vem.el.querySelector('.o_inner_group .o_web_studio_hook'));
        assert.strictEqual(editViewCount, 0,
            "the group cannot be dropped inside a group");
    });
